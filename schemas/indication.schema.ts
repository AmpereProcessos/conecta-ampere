import { z } from "zod";
import { SaleCategoryEnumSchema } from "./enums.schema";
...
export const IndicationSchema = z.object({
	...
	oportunidade: z.object({
		...
		dataGanho: z.string({ invalid_type_error: "Tipo não válido para data de ganho." }).datetime({ message: "Formato inválido para data de ganho." }).optional().nullable(),
		dataPerda: z.string({ invalid_type_error: "Tipo não válido para data de perda." }).datetime({ message: "Formato inválido para data de perda." }).optional().nullable(),
		dataInteracao: z
...
export type TIndication = z.infer<typeof IndicationSchema>;
export type TIndicationDTO = TIndication & { _id: string };

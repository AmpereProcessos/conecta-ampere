...
function UserIndicationCard({ indication }: UserIndicationCardProps) {
	function getIndicationStatusTag() {
		if (indication.oportunidade.dataGanho) {
			return <h1 className="rounded-full bg-green-500 p-1 font-extrabold text-[0.55rem] text-white">VENDA CONCLÚIDA</h1>;
		}
		if (indication.oportunidade.dataPerda) {
			return <h1 className="rounded-full bg-red-500 p-1 font-extrabold text-[0.55rem] text-white">VENDA PERDIDA</h1>;
		}
		if (indication.oportunidade.dataInteracao) {
			return <h1 className="rounded-full bg-blue-500 p-1 font-extrabold text-[0.55rem] text-white">EM ANDAMENTO</h1>;
		}

		...
		<div className="bg-[#fff] dark:bg-[#121212] w-full flex p-3.5 flex-col gap-1.5 shadow-sm border border-primary/20 rounded-lg">
			...
			<div className="w-full flex flex-col items-center justify-center grow py-3 px-0 lg:px-6 gap-1.5">
				<GeneralPaginationComponent
					...
					itemsMatchedText={
						indicationsMatched > 0
							? `${indicationsMatched} ordens de serviço encontradas.`
							: `${indicationsMatched} ordem de serviço encontrada.`
					}
					itemsShowingText={
						indicationsShowing > 0
							? `Mostrando ${indicationsShowing} ordens de serviço.`
							: `Mostrando ${indicationsShowing} ordem de serviço.`
					}
				/>
				{isLoading ? (
				...
				{isSuccess ? (
					indications && indications.length > 0 ? (
						...
						<>
							...
							<Button
								onClick={() => setNewIndicationMenuIsOpen((prev) => !prev)}
								size={"sm"}
								className="bg-blue-600 hover:bg-[#15599a]"
							>

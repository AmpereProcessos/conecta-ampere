		...
		<div className="flex w-full flex-col gap-1.5 py-4">
			...
			<div className="flex w-full flex-col items-center gap-1.5 px-6 py-3">
				{rewardOptions?.map((option) => (
					<button
						className="relative flex h-[150px] w-full cursor-pointer flex-col items-center justify-center gap-1 rounded-lg border-2 border-transparent p-2 transition-all duration-300 ease-in-out hover:border-primary lg:h-[250px] lg:p-8"
						key={option._id}
						onClick={() =>
							updateInfoHolder({
								recompensaResgatada: {
					...
					>
						{option.imagemCapaUrl && (
							<>
								...
								/>
								<div className="absolute inset-0 rounded-lg bg-primary-foreground/40" />
							</>
						)}

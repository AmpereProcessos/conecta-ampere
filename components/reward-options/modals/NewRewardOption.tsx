...
import { createRewardOption } from '@/lib/mutations/reward-options';
...
function NewRewardOption({ sessionUser, closeModal, callbacks }: NewRewardOptionProps) {
	...
	async function handleCreateRewardOptionWithUpload({ rewardOption, attachment }: { rewardOption: TRewardOption; attachment: TSimpleAttachment }) {
		...
		return createRewardOption({ payload });
	}
	const { mutate: mutateCreateIndication, isPending } = useMutation({
		mutationKey: ['create-reward-option'],
		mutationFn: handleCreateRewardOptionWithUpload,
		onMutate: () => {
	...
	const TITLE = 'NOVA RECOMPENSA';
	const DESCRIPTION = 'Preencha alguns dados sobre a sua recompensa.';
	const BUTTON_TEXT = 'CRIAR RECOMPENSA';
		...
		<Dialog onOpenChange={closeModal} open>
			<DialogContent className="flex h-fit max-h-[80vh] min-h-[60vh] flex-col">
				...
				<DialogFooter>
					...
					<LoadingButton loading={isPending} onClick={() => mutateCreateIndication({ rewardOption: infoHolder, attachment: imageHolder })}>
						{BUTTON_TEXT}
					</LoadingButton>
				</DialogFooter>
			</DialogContent>
...
export default NewRewardOption;

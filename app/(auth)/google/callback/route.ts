...
import { GOOGLE_OAUTH_STATE_COOKIE_NAME, GOOGLE_OAUTH_VERIFIER_COOKIE_NAME, type GoogleUserOpenIDConnect, google } from '@/lib/authentication/providers';
...
export async function GET(request: NextRequest): Promise<Response> {
	...
	try {
		...
		await interactionEventsCollection.insertOne({
			tipo: 'INSCRIÇÃO',
			localizacao: {
				uf: userRequestLocation.countryRegion || undefined,
				cidade: userRequestLocation.city?.toUpperCase() || undefined,
				latitude: userRequestLocation.latitude || undefined,
				longitude: userRequestLocation.longitude || undefined,
			},
			usuario: {
				id: clientId,
				nome: googleUser.name,

...
async function VerifyWaitingPage({
	searchParams,
}: { searchParams: { id: string; error?: string; details?: string } }) {
	const searchParamsValues = await searchParams;

	console.log(searchParamsValues);
	if (searchParamsValues.error)
		return <ErrorComponent msg={searchParamsValues.error} />;
	...
	if (!token)
		return <ErrorComponent msg="Oops, código não encontrado ou expirado." />;
				...
				<Card className="w-full max-w-md">
					...
					<CardContent className="gap-4 flex flex-col">
						{searchParamsValues.details ? (
							<p className="w-full text-sm tracking-tight text-blue-800 text-center font-bold">
								{searchParamsValues.details}
							</p>
						) : null}
					...
					<CardFooter>
						<Button variant={"link"} size={"sm"} className="p-0" asChild>
							<Link href={`/magic-link/send?userId=${token.usuarioId}`}>
